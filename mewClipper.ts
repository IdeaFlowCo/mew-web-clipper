...
// Initialize the Mew API with the user ID
initializeMewApi().catch(err => console.error("Failed to initialize MewAPI:", err));

// Function to set the current user ID in the MewAPI instance
...
async function getUserId(): Promise<string> {
  ...
  // IMPORTANT: Remove the 'user-root-id-' prefix to get the actual user ID
...
export async function getArticleNode(
  ...
  console.log(
    "[MewClipper] Creating new article node for", title
  );
  const response = await mewApi.addNode({
