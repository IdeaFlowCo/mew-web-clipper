...
export class MewAPI {
  ...
  async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(
          `Auth0 request failed: ${response.status} ${response.statusText}`
        );
        logger.error(`Response: ${errorText}`);
        throw new Error(
          `Auth failed: ${response.statusText}. Response: ${errorText}`
        );
      }
  ...
  async addNode(input: {
    ...
    if (relationLabel) {
      ...
      updates.push({
        ...
        node: {
          ...
          createdAt: timestamp,
          updatedAt: timestamp,
          content: [{ type: "text", value: relationLabel, styles: 0 }],
          isPublic: true,
          isNewRelatedObjectsPublic: false,
          canonicalRelationId: null,
          isChecked: null,
    ...
    if (serialized !== JSON.stringify(deserialized)) {
      logger.error(
        "WARNING: Payload may contain circular references or non-serializable values"
      );
    }
